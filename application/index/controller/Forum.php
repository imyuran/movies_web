...
use app\common\model\Askanswer as AskanswerModel;
   ...
   function check()
   {
      if(request()->isPost()){
        $data=input('post.');
          ...
          $askanswer = new AskanswerModel();

          $data = input('post.');
          $file = input('files.');

          // dump($file);
          $askanswer->userid     = session('userid');
          $askanswer->title      = $data['title'];
          $askanswer->created_at = date('Y-m-d H:i:s', time());
          // $askanswer->uploads = $data['file'];//上传有问题，暂设固定值
          $askanswer->uploads    = "/uploads/20171107/1512.jpg";
          $askanswer->desc       = $data['content'];
          $askanswer->save();
